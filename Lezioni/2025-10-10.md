...
Supponiamo per assurdo che $L$ lo sia. Sia $w$ la stringa $0^{p}1^{p}$ dove $p$ è il valore del pumping. 

Siccome $w\in L$ e $|w|\ge p$, per il pumping lemma la stringa $w$ deve poter essere divisa in tre pezzi $xyz$ dove per ogni $i\ge 0$ la stringa $xy^{i}z$ è in $L$. Consideriamo ciascuno dei casi in cui $w$ può essere diviso:
1) La stringa $y$ contiene solo zeri:
$$w=\underset{x}{\underbrace{0 ..}}\underset{y}{\underbrace{ ..}}01 ...1$$
In questo caso, la stringa $xyyz$ ha più zeri che uni, quindi non appartiene a $L$, violando così la prima condizione del lemma. Infatti, per $i\ge 2$ otterremmo $\hat{w}=xy^{i}z=0^{q}1^{p} \not\in L$ perché $q>p$. Questo contraddice il pumping lemma.
2) La stringa $y$ contiene solo uni:
$$w=\underset{x}{\underbrace{0 ..}}1\underset{y}{\underbrace{1..}}..1$$
Il caso è analogo alla situazione 1.
3) La stringa $y$ contiene degli zeri e degli uni
$$w=\underset{x}{\underbrace{0 ..}}\underset{y}{\underbrace{01..}}..1$$
In questo caso, la stringa $xyyz$ può avere lo stesso numero di $0$ e $1$, ma saranno in disordine con alcuni $1$ prima di alcuni $0$. Per cui $w\notin L.$

Dato che in tutti i casi è impossibile provare che $L$ sia regolare, $L$ non è regolare. Si noti che utilizzare la condizione $3$ del pumping lemma permette di non dover verificare i casi $2$ e $3$.

**Esercizio:**
Sia $L=\{w\in \{0,1 \}^{*}:\#_{0}w=\#_{1}w\}$ , ovvero $w$ ha un numero uguale di $0$ e $1$ in qualsiasi ordine. Assumiamo per assurdo che $L$ sia regolare. 

Supponiamo per assurdo che $L$ lo sia. Sia $w$ la stringa $0^{p}1^{p}$ dove $p$ è il valore del pumping.

Siccome $w\in L$ e $|w|\ge p$, per il pumping lemma la stringa $w$ deve poter essere divisa in tre pezzi $xyz$ dove per ogni $i\ge 0$ la stringa $xy^{i}z$ è in $L$.

In questo caso però ci accorgiamo che è possibile farlo. Se scegliamo $x=z=\mathcal{E}$ e $y=0^{p}1^{p}$ la stringa $xy^{i}z$ ha sempre un numero uguale di $0$ e di $1$, perciò appartiene ad $L$. Quindi sembrerebbe possibile "pomparla". 

Tuttavia qui torna utile la condizione $3$ del pumping lemma ($|xy|\le p$). Infatti, applicando questa condizione notiamo che la stringa $y$ deve essere composta da soli $0$, quindi $xyyz\ne L$. 

Perciò, $w$ non può essere pompata e otteniamo la contraddizione desiderata.

Notiamo però che utilizzando invece la stringa  $w=(01)^{p}$ non avremmo potuto ottenere una contraddizione perché può essere pompata rispettando tutte e 3 le condizioni del pumping lemma. Un modo per pomparla è $x=\mathcal{E},y=01,z=(0,1)^{p-1}.$ Infatti, $xy^{i}z\in L$.

Notiamo attentamente che se la stringa scelta può essere pompata non significa necessariamente che il linguaggio sia regolare, perché potrebbe esisterne un'altra che non può essere pompata.

Il problema quindi sta nel capire come scegliere la stringa da provare. Ad esempio, può essere utile concentrarsi su stringhe che richiederebbero all'automa di dover memorizzare una quantità infinita di dati (come ricordarsi di aver letto $n$ caratteri di un certo tipo dove $n$ non ha un limite superiore).

Durante gli esercizi, se sospetti che il linguaggio invece sia regolare, per dimostrarlo puoi provare a:
- Costruire un DFA che lo riconosca
- Scrivere un'espressione regolare che lo descrive
