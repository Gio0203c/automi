...
Se invece dell'unione avessimo analizzato l'intersezione $L=L_{1}\cap L_{2}$ avremmo dovuto usare $F_{1}\times F_{2}$, perché è necessario che nella coppia $(r_{1},r_{2})$, $r_{1}$ sia stato finale di $M_{1}$ e $r_{2}$ di $M_{2}$.
...
<span style="background:#affad1">La dimostrazione è stata data dal prof come esercizio.</span> <span style="background:#b1ffff">Svolgiamolo!</span>
...
Siccome la $\delta^{*}$ sarà rappresentata da una coppia $(p,q)$ avremo che
...
**$\Longleftarrow:$**
Se $x\in L_{1}$, avremo $\delta^{*}(q_{0}^{1},x)\in F_{1}$. Ma la $\delta^{*}$ è identificata dalla coppia
...
<span style="background:#affad1">Esercizio:</span> Dimostrare che $REG$ è chiusa per COMPLEMENTO e per INTERSEZIONE. 
