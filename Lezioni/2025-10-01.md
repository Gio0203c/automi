...
$$F=(F_{1}\times Q_{2})\cup(F_{2}\times Q_{1})$$
...
Si dimostra per induzione la seguente proprietà:
...
Una dimostrato questo, la correttezza segue direttamente dalla definizione degli stati finali.
...
**Dimostrazione:**
$\implies:$
Se $x\in L(M)$, allora $\delta^{*}(q_{0},x)\in F$, dove $F=(F_{1}\times Q_{2})\cup (F_{2}\times Q_{1})$.
